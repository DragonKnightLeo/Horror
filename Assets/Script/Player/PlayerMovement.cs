...
>>>>>>> Stashed changes
    ...
    */
    ...
    {
        if (canShoot && !isMoving && Time.time >= nextAttackTime)
        {
    ...
    */
    ...
    void playFootSteps()
    {
        SoundManager.instance.footSteps(0.7f);
    }

    void playRunningSteps()
    {
        SoundManager.instance.runningSteps(0.7f);
    }
    ...
    public void adjustIntensity(float intensity)
    {
        for(int i = 0; i < lightBoxes.Length; i++)
        {
            lightBoxes[i].intensity = intensity;
        }
